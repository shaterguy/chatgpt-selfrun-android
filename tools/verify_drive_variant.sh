...
grep -Fq 'SelfRunServerFeaturePolicy.buildEnabled()' "$COORD"
...
for field in TASK_ID TURN_ID REQUEST_ID RESULT_DOCUMENT_ID REQUIREMENT_DOCUMENT_ID PREVIOUS_RESULT_DOCUMENT_ID; do
  grep -Fq "$field" "$CONTRACT"
done
for removed in 'TURN"' PHASE TASK_MODE 'MODE"' EXECUTION_KIND SIGNAL_TYPE FOLDER_ID RECEIPT EXECUTION_PROFILE RESULT_DOCUMENT_RULES '[RESULT_DOCUMENT_CONTRACT]' SELF_RUN_V3; do
...
grep -Fq 'committedResultBeyondFormer512KiBGateKeepsMechanicalIdentityGate' "$UNBOUNDED_TEST"
