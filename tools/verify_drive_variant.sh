...
TURN_BRIDGE=$SRC/TurnProtocolLogBridge.java
WEB_CONFIG=$SRC/WebViewConfig.java
HEADLESS_HOST=$SRC/HeadlessWebViewHost.java
...
grep -Fq 'HybridRunProfileStore.MODE_HYBRID' "$ACTIVITY"
grep -Fq 'hybridBootstrapProfile' "$ACTIVITY"
grep -Fq 'hybridContinuationProfile' "$ACTIVITY"
grep -Fq 'HybridRequestProfileScript.installDocumentStart' "$SRC/WebViewConfig.java"
grep -Fq 'SELF_RUN_HYBRID_STAGE=' "$SRC/HybridRunProfileStore.java"
...
! grep -Fq 'TURN_COMPLETION_STABILITY_MS' "$SERVICE"
...
! grep -Fq 'observeTurnCompletion' "$SERVICE"
! grep -Fq 'TURN_OBSERVER_HEALTHCHECK_MS' "$SERVICE"
! grep -Fq 'recoverDetachedObserverOutput' "$SERVICE"
grep -Fq 'SelfRunContinuationDom.clickPreparedDriveTurn' "$SERVICE"
grep -Fq 'SelfRunContinuationDom.clickPreparedBootstrap' "$SERVICE"
grep -Fq 'PHASE_WAIT_INTERNAL_SEND' "$STORE"
grep -Fq 'PHASE_POST_PROTOCOL_DRIVE_SYNC' "$STORE"
grep -Fq 'beginPostProtocolDriveSync' "$STORE"
! grep -Fq 'command_received_ack' "$SERVICE"
! grep -Fqi 'Command Received Record Required' "$PROTOCOL"
...
grep -Fq 'COMPOSER_IDLE' "$CONTINUE_DOM"
...
! grep -Fq 'new MutationObserver' "$CONTINUE_DOM"
! grep -Fq '__selfRunDriveTurnObserver' "$TURN_PROTOCOL"
! grep -Fq '__selfRunDriveTurnObserver' "$CONTINUE_DOM"
! test -e "$SRC/TurnCompletionDomFallbackScript.java"
grep -Fq 'turnToken' "$TURN_PROTOCOL"
...
grep -Fq 'TURN_PROTOCOL_UNAVAILABLE' "$SERVICE"
grep -Fq 'virtualDisplay.setSurface(null)' "$HEADLESS_HOST"
grep -Fq 'virtualDisplay.setSurface(surface)' "$HEADLESS_HOST"
grep -Fq 'detachDisplayOutput("submission_confirmed")' "$SERVICE"
grep -Fq 'data-message-author-role' "$CONTINUE_DOM"
grep -Fq 'pollDriveNow' "$SERVICE"
