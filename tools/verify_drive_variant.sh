...
grep -Eq 'case "PAUSED"[[:space:]]*->[[:space:]]*finishPersistedTerminalPause\("DRIVE_PAUSED",[[:space:]]*"일시정지"' "$SERVICE"
grep -Eq 'case "USER_ACTION_REQUIRED"[[:space:]]*->[[:space:]]*finishPersistedTerminalPause\("DRIVE_USER_ACTION_REQUIRED",[[:space:]]*"확인 필요"' "$SERVICE"
