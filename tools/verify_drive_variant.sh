...
grep -Fq 'SUBMISSION_RECEIPT_TIMEOUT_MS = 120_000L' "$ENGINE"
...
grep -Fq 'canonicalPostStarted' "$ENGINE"
grep -Fq 'receiptDeadlineElapsed' "$ENGINE"
grep -Fq 'acceptanceProof' "$ENGINE"
...
grep -Fq 'canonicalPostStartDoesNotMeanAccepted' "$ENGINE_TEST"
grep -Fq 'positiveConversationProofMovesDispatchToWaiting' "$ENGINE_TEST"
grep -Fq 'receiptDeadlineIsFiniteAcrossSameBootAndWallFallback' "$ENGINE_TEST"
grep -Fq 'legacyGhostWaitingStateNormalizesToUncertainDispatchWithoutIdentityLoss' "$ENGINE_TEST"
...
grep -Fq 'stoppedTaskRequiresExplicitRecoveryAndPreservesAcceptedSendClaim' "$ENGINE_TEST"

grep -Fq 'DEFAULT_RESULT_REPAIR_MINUTES * 60_000L' "$WATCHDOG"
grep -Fq 'state.stage() != SelfRun3Engine.Stage.WAITING' "$WATCHDOG"
grep -Fq 'resultBodyMutationObservedElapsed' "$WATCHDOG"
grep -Fq 'resultBodyMutationBootCount' "$WATCHDOG"
...
grep -Fq 'SelfRun3Engine.receiptDelayMs' "$COORD"
grep -Fq 'V3_SUBMISSION_OUTCOME_UNCERTAIN' "$COORD"
grep -Fq '"proof", "conversation_url"' "$COORD"
...
grep -Fq 'stoppedUncertainDispatchSurvivesProjectionRestartAndResumeEventIsIdempotent' "$STOPPED_RESUME_ANDROID_TEST"
