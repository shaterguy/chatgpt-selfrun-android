...
for file in "$SERVICE" "$COORD" "$ENGINE" "$LEDGER" "$CONTRACT" "$DRIVE" "$LOOKUP" "$WEB" "$DISPATCH" "$BOOTSTRAP" "$POWER" "$STRICT" "$MARKER" "$INPUT"; do
  test -s "$file"
done
...
grep -Fq 'return nativeFetch(input,init)' "$DISPATCH"
grep -Fq 'return send.call(this,body)' "$DISPATCH"
