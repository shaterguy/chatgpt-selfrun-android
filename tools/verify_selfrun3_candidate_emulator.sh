...
adb install -r previous/chatgpt-selfrun-drive-test-v3.0.1-dev9.apk >/dev/null
[[ "$(adb shell dumpsys package "$TEST" | tr -d '\r' | sed -n 's/^[[:space:]]*versionName=//p' | head -1)" == '3.0.1-dev9' ]]
...
[[ "$OLD_UID" == "$NEW_UID" ]]
printf 'TEST_UPGRADE=PASS;from=3.0.1-dev9;to=%s;uid_preserved=true\n' "$VERSION_NAME" | tee selfrun-v3-runtime-evidence.txt
