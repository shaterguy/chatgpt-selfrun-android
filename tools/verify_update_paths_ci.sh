...
stable_package_dump() {
  ...
  for i in $(seq 1 15); do
    if dump="$(adb shell dumpsys package "$PKG" 2>/dev/null | tr -d '\r')"; then
      if grep -Fq "Package [$PKG]" <<<"$dump"; then printf '%s\n' "$dump"; return 0; fi
    fi
    sleep 1
  done
  return 1
...
package_uid() {
  adb_ready
  local out uid
  out="$(adb shell cmd package list packages -U "$PKG" 2>/dev/null | tr -d '\r')"
  uid="${out##*uid:}"
  uid="${uid%% *}"
  uid="${uid%%$'\n'*}"
  [[ "$uid" =~ ^[0-9]+$ ]]
...
scenario() {
  ...
  snapshot_package
  [[ "$SNAP_VERSION_NAME" == "$expected_base" && "$SNAP_VERSION_CODE" == "$expected_code" ]]
  ...
  snapshot_package
  [[ "$SNAP_VERSION_NAME" == '1.2.1' && "$SNAP_VERSION_CODE" == '1000021' ]]
  [[ "$SNAP_UID" == "$before_uid" && "$SNAP_DATA_DIR" == "$before_dir" && "$SNAP_FIRST_INSTALL" == "$before_first" ]]
  notification_granted
  launch_app "$label"
