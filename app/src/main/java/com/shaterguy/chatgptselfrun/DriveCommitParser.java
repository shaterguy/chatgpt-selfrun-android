...
final class DriveSignalParser {
 ...
 static Scan scan(String text,String jobId,int consumed){return scan(text,jobId,consumed,SelfRunStore.MODE_CHAT);}
 static Scan scan(String text,String jobId,int consumed,String mode){boolean work=SelfRunStore.MODE_WORK.equals(mode);List<Event> all=new ArrayList<>();for(String line:(text==null?"":text).split("\\r?\\n")){Matcher m=LINE.matcher(line.trim());if(!m.matches()||!jobId.equals(m.group(3)))continue;Type t=type(m.group(2));String tail=m.group(4)==null?"":m.group(4).trim();if(!tail.isEmpty()&&(t!=Type.TURN_COMPLETED||!work))continue;all.add(new Event(t,m.group(1),m.group(0),all.size()+1));}int requested=Math.max(0,consumed);boolean rebased=requested>all.size();int base=Math.min(requested,all.size());List<Event> unseen=base>=all.size()?Collections.emptyList():new ArrayList<>(all.subList(base,all.size()));return new Scan(Collections.unmodifiableList(unseen),all.size(),all.isEmpty()?null:all.get(all.size()-1),rebased);}
