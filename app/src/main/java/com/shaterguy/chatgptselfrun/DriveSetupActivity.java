...
/** One-time drive.file + Google Picker binding for the existing Runs folder. */
public final class DriveSetupActivity extends Activity {
    ...
    private void render() {
        ...
        root.addView(Ui.body(this, "Google Picker에서 /GPT/Project/Vibe Coding/00_System/SelfRun/Runs/ 폴더를 선택합니다. 앱은 drive.file 범위만 요청하며 선택된 폴더와 앱이 그 아래에 만드는 항목만 사용합니다."));
        status = Ui.body(this, bindingSummary());
        root.addView(status);
        root.addView(Ui.row(this,
                Ui.button(this, "Drive 실행문서 저장 위치 연결", v -> startPicker()),
                Ui.button(this, "연결 해제", v -> unbind())));
    ...
    private String bindingSummary() {
        if (store.driveRunsBaseFolderId().isEmpty()) return "현재 바인딩: 없음";
        return "현재 바인딩: " + store.driveRunsBaseFolderName()
    ...
    private void startPicker() {
        ...
        DriveAuthorization.requestFolderPicker(this, new DriveAuthorization.Callback() {
            ...
            @Override public void onResolutionRequired(PendingIntent pendingIntent) {
                try {
                    startIntentSenderForResult(pendingIntent.getIntentSender(), REQUEST_PICK_FOLDER,
                            null, 0, 0, 0);
