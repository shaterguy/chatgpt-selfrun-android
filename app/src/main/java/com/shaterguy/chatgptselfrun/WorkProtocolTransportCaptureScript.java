        ...
        return """
                (()=>{
                  ...
                  const diagnostic=(stage,fields={})=>{
                    if(!workMode())return;
                    try{
                      const sink=window.selfRunTurnLog,id=runId();if(!sink||typeof sink.postMessage!=='function'||!id)return;
                  ...
                  const recordMainCanonical=source=>{
                    if(!mainFrame()||!workMode())return false;emitEnvironment();
                    admission={runId:runId(),at:Date.now(),source};
                  ...
                  const acceptCanonical=(source,expectedRun)=>{
                    ...
                    diagnostic('WORK_PROTOCOL_TRANSPORT',{source,transport:source,route:'canonical_conversation',outcome:'canonical_request'});
                  ...
                  const minimalSemantic=node=>{
                    ...
                    if(node.end_turn===true)out.end_turn=true;
                    if(node.author?.role)out.author={role:token(node.author.role,32)};if(node.channel)out.channel=token(node.channel,32);if(node.id)out.id=safe(node.id);
                    const message=minimalMessage(node.message);if(message)out.message=message;
                    const delta=minimalMessage(node.v?.message);if(delta)out.v={message:delta};
                    if(!out.message&&!out.v?.message&&out.author?.role)out.message=minimalMessage(node);
                  ...
                  const relay=(kind,payload,source)=>{
                    ...
                    try{window.top.postMessage({relay:RELAY,version:1,runId:runId(),kind,source:token(source,48),payload},relayOrigin());return true;}
                    catch(_){return false;}
                  };
                  const handleRelayedSemantic=(payload,source,expectedRun)=>{
                    if(!mainFrame()||!workMode()||safe(expectedRun)!==runId())return false;
                    const minimal=minimalSemantic(payload);if(!minimal)return false;emitEnvironment();
                    ...
                    try{p.observeSseText('data: '+JSON.stringify(minimal)+'\\n\\n','work-subframe-relay',context);}
                    catch(_){diagnostic('WORK_PROTOCOL_DECODE_ERROR',{source,reason:'semantic_relay_failed'});return false;}
                    ...
                    if(before!==after)diagnostic('WORK_PROTOCOL_TRANSITION',{source,transition:before+'>'+after,completion:safe(p.snapshot?.()?.completionSource||'')});
                    return true;
                  };
                  if(mainFrame()){
                    window.addEventListener('message',event=>{
                      const data=event.data;if(!workMode()||event.origin!==location.origin||!event.source||event.source===window
                        ||!data||data.relay!==RELAY||data.version!==1||safe(data.runId)!==runId())return;
                      if(data.kind==='canonical'){acceptCanonical('subframe_'+token(data.source,36),data.runId);return;}
                      if(data.kind==='semantic')handleRelayedSemantic(data.payload,'subframe_'+token(data.source,36),data.runId);
                    });
                    ...
                    const p=protocol();if(p&&typeof p.observeSseText==='function'){
                      const downstream=p.observeSseText.bind(p);
                      p.observeSseText=function(text,source,context){
                        if(workMode()&&String(source||'').startsWith('work-decoder-')){
                          const node=parseSemanticSse(text),minimal=minimalSemantic(node);if(minimal&&relay('semantic',minimal,source))return p.snapshot?.()||{};
                        }
                  ...
                  if(typeof downstreamFetch==='function'){
                    const wrappedFetch=function(...args){const meta=requestMeta(args[0],args[1]);
                      if(workMode()&&canonicalConversation(meta.method,meta.url)){if(mainFrame())recordMainCanonical('main_fetch');else relay('canonical',{route:'canonical_conversation'},'fetch');}
                      return Reflect.apply(downstreamFetch,this,args);};
                    try{Object.defineProperty(wrappedFetch,'name',{value:downstreamFetch.name});Object.defineProperty(wrappedFetch,'length',{value:downstreamFetch.length});wrappedFetch.toString=downstreamFetch.toString.bind(downstreamFetch);}catch(_){}
                    window.fetch=wrappedFetch;
                  }
                  const downstreamOpen=XMLHttpRequest.prototype.open,downstreamSend=XMLHttpRequest.prototype.send,xhrMeta=new WeakMap();
                  XMLHttpRequest.prototype.open=function(method,url,...rest){xhrMeta.set(this,{method:String(method||''),url:String(url||'')});return downstreamOpen.call(this,method,url,...rest);};
                  ...
                  const observeServiceWorkerPortData=data=>{if(!mainFrame()||!workMode())return false;emitEnvironment();
                    diagnostic('WORK_PROTOCOL_TRANSPORT',{source:'service_worker_message_port',transport:'service_worker_message_port',outcome:'message_received'});
                    try{void ingress()?.observeTransportData?.(data,'service_worker_message_port');return true;}catch(_){return false;}};
                  window.__selfRunWorkProtocolTransportCapture={version:ENGINE_VERSION,observeNativeCanonical,
                    observeServiceWorkerData,observeServiceWorkerPortData,
                    observeRelayedSemantic:(payload,source='fixture')=>handleRelayedSemantic(payload,source,runId()),
                    diagnostics:()=>({mode:safe(target()?.mode),mainFrame:mainFrame(),runId:runId(),admission:{...admission}})};
                  emitEnvironment();
                })();
