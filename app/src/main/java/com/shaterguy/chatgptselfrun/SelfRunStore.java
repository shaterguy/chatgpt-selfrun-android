...
private void startLocked(String runId,String mode,String projectUrl,String requirement){
 ...
 commitOrThrow(prefs.edit().putString("runId",safe(runId)).putLong("createdAt",now).putLong("phaseStartedAt",now)
  ...
  .putString("pendingDriveSignalRaw","").putString("pendingDriveSignalTimestamp","").putString("pendingDriveSignalType","").putInt("pendingDriveSignalCursor",0).putLong("commitDetectedAt",0L).putLong("guardDueAt",0L).putString("completionGuardFingerprint","").putBoolean("completionGuardArmed",false).putBoolean("rewriteCarryAuthorized",false).putBoolean("driveRebaselineAuthorized",false)
...
void baselineDriveSignals(int cursor,DriveSignalParser.Event latest){SharedPreferences.Editor e=prefs.edit().putInt("driveSignalCursor",Math.max(0,cursor)).putBoolean("driveRebaselineAuthorized",false);putLatest(e,latest);commitOrThrow(e);syncHistory();}
...
static final class ResumeDriveTransaction{
 ...
 void observe(List<DriveSignalParser.Event> events,int anchorCursor,int totalCount){if(structuralError)return;if(consumedCursor<0)consumedCursor=Math.max(0,anchorCursor);if(events==null)return;for(DriveSignalParser.Event x:events){if(x==null||x.cursor<=anchorCursor||x.cursor>totalCount){error="RESUME_POST_ANCHOR_SIGNAL_INVALID";errorEvent=null;material=null;rank=3;structuralError=true;return;}consumedCursor=x.cursor;lastProcessed=x;if(x.type==DriveSignalParser.Type.TURN_COMPLETED&&guardArmed&&DriveSignalParser.completionFingerprint(x.raw).equals(guardFingerprint))continue;if(awaiting){if(x.type==DriveSignalParser.Type.COMMAND_RECEIVED){awaiting=false;acked=true;if(RETRY_CONTINUE.equals(kind)&&rewrite){rewriteAck=true;pending.authorizeCarry();}else if(RETRY_CONTINUE.equals(kind)){normalAck=true;pending.supersede();guardArmed=false;guardFingerprint="";}continue;}awaiting=false;setProtocolError("COMMAND_RECEIVED_REQUIRED",x);continue;}if(x.type==DriveSignalParser.Type.INVALID){setProtocolError(x.protocolError.isEmpty()?"RESUME_PROTOCOL_INVALID":x.protocolError,x);continue;}switch(x.type){case COMMAND_RECEIVED->{}case TURN_COMPLETED->{if(rank<2){rank=2;material=x;guardArmed=true;guardFingerprint=DriveSignalParser.completionFingerprint(x.raw);}}case USER_ACTION_REQUIRED,PAUSED,DONE->{setTerminal(x);}case INVALID->throw new IllegalStateException("handled above");}}}
...
void repairGuard(long now,long guardMs){
...
void baselineManualResume(List<DriveSignalParser.Event> postAnchor,int totalCount,DriveSignalParser.Event latest){
 ...
 DriveResumePolicy.Decision decision=DriveResumePolicy.decide(origin,resumeNeedsContinuation(),pauseAnchorCursor(),resumeCursor,tx.policyEvents());
...
private SharedPreferences.Editor invalidateSupersededContinuation(SharedPreferences.Editor e){SelfRunProtocol.clearPendingContinuation(runId());clearCommandWait(e);clearPendingCompletion(e);resetCompletionGuard(e);e.putBoolean("rewriteCarryAuthorized",false).putBoolean("driveRebaselineAuthorized",false);return e;}
...
private void terminal(SharedPreferences.Editor e,DriveSignalParser.Event x){e.putBoolean("terminalSideEffectPending",true).putString("terminalSideEffectType",x.type.name()).putString("terminalSideEffectRunId",runId()).putString("terminalSideEffectCommitId",x.raw);}
    ...
    private static SharedPreferences.Editor clearPendingCompletion(SharedPreferences.Editor e){return e.putString("pendingDriveSignalRaw","").putString("pendingDriveSignalTimestamp","").putString("pendingDriveSignalType","").putInt("pendingDriveSignalCursor",0).putLong("commitDetectedAt",0L).putLong("guardDueAt",0L).putBoolean("rewriteCarryAuthorized",false);}
