...
final class SelfRunStore {
    ...
    static final String PHASE_WAIT_INTERNAL_SEND = "WAIT_INTERNAL_SEND";
...
void applyDriveSignals(List<DriveSignalParser.Event> events,long detectedAt){if(events==null||events.isEmpty())return;SharedPreferences.Editor e=prefs.edit();int rank=PHASE_DONE.equals(phase())||PHASE_PAUSED.equals(phase())?3:PHASE_WAIT_INTERNAL_SEND.equals(phase())?2:0;for(DriveSignalParser.Event x:events){e.putInt("driveSignalCursor",x.cursor);putLatest(e,x);switch(x.type){case COMMAND_RECEIVED->{if(rank<2)e.putString("status","Drive COMMAND_RECEIVED 확인 · 작업 진행 중");}case TURN_COMPLETED->{if(!x.protocolError.isEmpty())continue;if(rank<2){rank=2;e.putString("pendingDriveSignalRaw",x.raw).putString("pendingDriveSignalTimestamp",x.timestamp).putString("pendingDriveSignalType",x.type.name()).putLong("commitDetectedAt",detectedAt).putString("phase",PHASE_WAIT_INTERNAL_SEND).putString("status","Drive TURN_COMPLETED 확인 · 내부 WebView SEND 상태 확인");}}case USER_ACTION_REQUIRED->{rank=3;clearCommandWait(e);pauseEvent(e,x,"사용자 조치 필요");}case PAUSED->{rank=3;clearCommandWait(e);pauseEvent(e,x,"SelfRun Drive 일시정지");}case DONE->{rank=3;clearCommandWait(e);e.putBoolean("active",false).putBoolean("paused",false).putBoolean("resumeNeedsContinuation",false).putString("phase",PHASE_DONE).putString("status","SelfRun Drive 완료");terminal(e,x);}}}e.putBoolean("awaitingCommandAck",false).putLong("phaseStartedAt",System.currentTimeMillis());commitOrThrow(e);syncHistory();}
...
private void migrateLegacyDriveCommitGuard(){synchronized(RUN_STATE_LOCK){if(!"DRIVE_COMMIT_GUARD".equals(phase()))return;commitOrThrow(prefs.edit().putString("phase",PHASE_WAIT_INTERNAL_SEND).putString("status","업데이트된 TURN_COMPLETED · 내부 WebView SEND 상태 확인").putLong("phaseStartedAt",System.currentTimeMillis()));syncHistory();}}
