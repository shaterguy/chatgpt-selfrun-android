...
void beginTurnCompletionWait(String turnToken,String waitingStatus){String token=safe(turnToken);if(token.isEmpty()||!token.equals(turnProtocolToken()))throw new IllegalStateException("prepared protocol turn required");String appliedModel=pendingModel(),appliedReasoning=pendingReasoning();SharedPreferences.Editor e=clearWatchdogClaimFields(clearCommandWait(prefs.edit())).putString("pendingDriveSignalRaw","").putString("pendingDriveSignalTimestamp","").putString("pendingDriveSignalType","").putLong("commitDetectedAt",0L).putString("pendingModel",appliedModel).putString("pendingReasoning",appliedReasoning).putBoolean("turnProtocolCompletionConsumed",false).putString("turnProtocolCompletionSource","").putString("phase",PHASE_WAIT_TURN_COMPLETION).putString("status",safe(waitingStatus)).putLong("phaseStartedAt",System.currentTimeMillis()).putLong("postProtocolDriveSyncStartedAt",0L);commitOrThrow(e);syncHistory();}
