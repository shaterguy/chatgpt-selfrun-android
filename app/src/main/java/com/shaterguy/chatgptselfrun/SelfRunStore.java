...
private void startLocked(String runId,String mode,String projectUrl,String requirement,List<Attachment> attachments,String initialModel,String initialReasoning){
 ...
 syncHistory();
    ...
    boolean turnObserverSawStop() { return prefs.getBoolean("turnObserverSawStop", false); }
...
void prepareTurnObserver(String token){String value=safe(token);if(value.isEmpty())throw new IllegalArgumentException("turn observer token required");commitOrThrow(prefs.edit().putString("turnObserverToken",value).putBoolean("turnObserverSawStop",false));}
static boolean isActiveTurnObserverCallbackPhase(String phase){return PHASE_BOOTSTRAP_SEND.equals(phase)||PHASE_SEND_CONTINUE.equals(phase)||PHASE_WAIT_TURN_COMPLETION.equals(phase);}
boolean markTurnObserverStopSeen(String observerToken){String token=safe(observerToken);if(!isActiveTurnObserverCallbackPhase(phase())||token.isEmpty()||!token.equals(turnObserverToken()))return false;commitOrThrow(prefs.edit().putBoolean("turnObserverSawStop",true));syncHistory();return true;}
