...
void markCommandSubmitted(String kind,long due){if(!kind.equals(activeCommandKind())||activeCommandPrompt().isEmpty()||due<=0)throw new IllegalStateException("prepared command required");long now=System.currentTimeMillis();int n=submissionRetryAttempt()==Integer.MAX_VALUE?Integer.MAX_VALUE:submissionRetryAttempt()+1;SharedPreferences.Editor e=prefs.edit().putBoolean("awaitingCommandAck",true).putString("submissionRetryKind",kind).putString("submissionRetryReason","COMMAND_RECEIVED_PENDING").putLong("submissionRetryDueAt",due).putInt("submissionRetryAttempt",n).putBoolean("submissionRetryReady",false).putString("phase",PHASE_WAIT_DRIVE_COMMIT).putString("status","Drive COMMAND_RECEIVED 대기 · 5분 후 미수신 시 재제출").putLong("phaseStartedAt",now);if(RETRY_BOOTSTRAP.equals(kind))e.putLong("bootstrapSubmittedAt",now).putString("bootstrapSubmissionState",BOOTSTRAP_SUBMISSION_STARTED);commitOrThrow(e);syncHistory();}
...
boolean captureConversationUrl(String value){
    boolean submitted=bootstrapSubmittedAt()>0L&&(BOOTSTRAP_SUBMISSION_STARTED.equals(bootstrapSubmissionState())
            ||BOOTSTRAP_SUBMISSION_CONFIRMED.equals(bootstrapSubmissionState()));
    if(!ChatRoutePolicy.shouldCaptureBootstrapConversation(submitted,conversationUrl(),projectUrl(),value))return false;
    commitOrThrow(prefs.edit().putString("conversationUrl",safe(value))
            .putString("bootstrapSubmissionState",BOOTSTRAP_SUBMISSION_CONFIRMED));syncHistory();return true;
}
