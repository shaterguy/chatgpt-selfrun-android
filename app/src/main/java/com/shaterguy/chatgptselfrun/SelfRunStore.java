...
boolean markTurnObserverStopSeen(String observerToken){String token=safe(observerToken);if(!PHASE_WAIT_TURN_COMPLETION.equals(phase())||token.isEmpty()||!token.equals(turnObserverToken()))return false;commitOrThrow(prefs.edit().putBoolean("turnObserverSawStop",true));syncHistory();return true;}
