...
private void startLocked(String runId,String mode,String projectUrl,String requirement,List<Attachment> attachments){
 ...
 commitOrThrow(prefs.edit().putString("runId",safe(runId)).putLong("createdAt",now).putLong("phaseStartedAt",now)
  ...
  .putInt("driveSignalCursor",0).putString("turnObserverToken","").putBoolean("turnObserverSawStop",false).putLong("postDomDriveSyncStartedAt",0L).putString("lastDriveSignalRaw","").putString("lastDriveSignalTimestamp","").putString("lastDriveSignalType","")
