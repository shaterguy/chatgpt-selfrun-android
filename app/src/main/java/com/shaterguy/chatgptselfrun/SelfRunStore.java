...
private void startLocked(String runId,String mode,String projectUrl,String requirement,List<Attachment> attachments,String initialModel,String initialReasoning){
 ...
 commitOrThrow(prefs.edit().putString("runId",safe(runId)).putLong("createdAt",now).putLong("phaseStartedAt",now)
  ...
  .putInt("driveSignalCursor",0).putInt("driveSignalCursorSchemaVersion",DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL).putString("turnProtocolToken","").putBoolean("turnProtocolCompletionConsumed",false).putString("turnProtocolCompletionSource","").putLong("postProtocolDriveSyncStartedAt",0L).putString("lastDriveSignalRaw","").putString("lastDriveSignalTimestamp","").putString("lastDriveSignalType","")
 ...
 syncHistory();
    ...
    int driveSignalCursorSchemaVersion() { return prefs.getInt("driveSignalCursorSchemaVersion", DRIVE_SIGNAL_CURSOR_SCHEMA_LEGACY); }
    ...
    String turnProtocolToken() { return get("turnProtocolToken"); }
    boolean turnProtocolCompletionConsumed() { return prefs.getBoolean("turnProtocolCompletionConsumed", false); }
    String turnProtocolCompletionSource() { return get("turnProtocolCompletionSource"); }
    long postProtocolDriveSyncStartedAt() { return prefs.getLong("postProtocolDriveSyncStartedAt", 0L); }
    ...
    void finishWatchdogRecoveryBaseline(int cursor, DriveSignalParser.Event latest) {
        synchronized (RUN_STATE_LOCK) {
            ...
            SharedPreferences.Editor e = clearWatchdogClaimFields(prefs.edit())
                    .putInt("driveSignalCursor", Math.max(0, cursor)).putInt("driveSignalCursorSchemaVersion", DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL)
...
void baselineDriveSignals(int cursor,DriveSignalParser.Event latest){SharedPreferences.Editor e=prefs.edit().putInt("driveSignalCursor",Math.max(0,cursor)).putInt("driveSignalCursorSchemaVersion",DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL);putLatest(e,latest);commitOrThrow(e);syncHistory();}
void migrateDriveSignalCursor(int cursor){if(cursor<0)throw new IllegalArgumentException("physical cursor required");commitOrThrow(prefs.edit().putInt("driveSignalCursor",cursor).putInt("driveSignalCursorSchemaVersion",DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL));syncHistory();}
...
void prepareTurnProtocolToken(String token){String value=safe(token);if(value.isEmpty())throw new IllegalArgumentException("protocol turn token required");commitOrThrow(prefs.edit().putString("turnProtocolToken",value).putBoolean("turnProtocolCompletionConsumed",false).putString("turnProtocolCompletionSource",""));syncHistory();}
void bootstrapSubmissionConfirmed(String turnToken){if(!RETRY_BOOTSTRAP.equals(activeCommandKind())||activeCommandPrompt().isEmpty())throw new IllegalStateException("prepared bootstrap command required");beginTurnCompletionWait(turnToken,"첫 요청 제출 확인 · 응답 프로토콜 대기 중");}
void beginTurnCompletionWait(String turnToken,String waitingStatus){beginTurnCompletionWait(turnToken,waitingStatus,false);}
void beginTurnCompletionWait(String turnToken,String waitingStatus,boolean consumeLegacyNotice){String token=safe(turnToken);if(token.isEmpty()||!token.equals(turnProtocolToken()))throw new IllegalStateException("prepared protocol turn required");String appliedModel=pendingModel(),appliedReasoning=pendingReasoning();SharedPreferences.Editor e=clearWatchdogClaimFields(clearCommandWait(prefs.edit())).putString("pendingDriveSignalRaw","").putString("pendingDriveSignalTimestamp","").putString("pendingDriveSignalType","").putLong("commitDetectedAt",0L).putString("pendingModel",appliedModel).putString("pendingReasoning",appliedReasoning).putBoolean("turnProtocolCompletionConsumed",false).putString("turnProtocolCompletionSource","").putString("phase",PHASE_WAIT_TURN_COMPLETION).putString("status",safe(waitingStatus)).putLong("phaseStartedAt",System.currentTimeMillis()).putLong("postProtocolDriveSyncStartedAt",0L);if(consumeLegacyNotice)e.putBoolean("legacyModeNoticeConsumed:"+runId(),true);commitOrThrow(e);syncHistory();}
boolean beginPostProtocolDriveSync(String turnToken,String source){String token=safe(turnToken),completionSource=safe(source);if(!PHASE_WAIT_TURN_COMPLETION.equals(phase())||turnProtocolCompletionConsumed()||token.isEmpty()||!token.equals(turnProtocolToken())||!TurnProtocolLogBridge.isAllowedCompletionSource(completionSource))return false;long now=System.currentTimeMillis();commitOrThrow(prefs.edit().putBoolean("turnProtocolCompletionConsumed",true).putString("turnProtocolCompletionSource",completionSource).putString("phase",PHASE_POST_PROTOCOL_DRIVE_SYNC).putString("status","프로토콜 답변 완료 · Drive 신호 즉시 확인").putLong("phaseStartedAt",now).putLong("postProtocolDriveSyncStartedAt",now));syncHistory();return true;}
void applyDriveSignals(List<DriveSignalParser.Event> events,long detectedAt){if(events==null||events.isEmpty())return;SharedPreferences.Editor e=prefs.edit().putInt("driveSignalCursorSchemaVersion",DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL);int rank=PHASE_DONE.equals(phase())||PHASE_PAUSED.equals(phase())?3:0;for(DriveSignalParser.Event x:events){e.putInt("driveSignalCursor",x.cursor);putLatest(e,x);switch(x.type){case TURN_COMPLETED->{if(!x.protocolError.isEmpty())continue;if(rank<2){rank=2;String next=MODE_WORK.equals(mode())?PHASE_APPLY_PREFS:PHASE_SEND_CONTINUE;e.putString("pendingDriveSignalRaw",x.raw).putString("pendingDriveSignalTimestamp",x.timestamp).putString("pendingDriveSignalType",x.type.name()).putLong("commitDetectedAt",detectedAt).putLong("postProtocolDriveSyncStartedAt",0L).putString("phase",next).putString("status","Drive TURN_COMPLETED·NEXT_INPUT 확인 · 다음 턴 전송 준비");}}case USER_ACTION_REQUIRED->{rank=3;clearCommandWait(e);pauseEvent(e,x,"사용자 조치 필요");}case PAUSED->{rank=3;clearCommandWait(e);pauseEvent(e,x,"SelfRun Drive 일시정지");}case DONE->{rank=3;clearCommandWait(e);e.putBoolean("active",false).putBoolean("paused",false).putBoolean("resumeNeedsContinuation",false).putString("phase",PHASE_DONE).putString("status","SelfRun Drive 완료");terminal(e,x);}}}e.putBoolean("awaitingCommandAck",false).putLong("phaseStartedAt",System.currentTimeMillis());commitOrThrow(e);syncHistory();}
static boolean isManualResumeWebPhase(String phase){return PHASE_BOOTSTRAP.equals(phase)||PHASE_BOOTSTRAP_MODEL.equals(phase)||PHASE_BOOTSTRAP_REASONING.equals(phase)||PHASE_BOOTSTRAP_SEND.equals(phase)||PHASE_WAIT_TURN_COMPLETION.equals(phase)||PHASE_APPLY_PREFS.equals(phase)||PHASE_APPLY_REASONING.equals(phase)||PHASE_SEND_CONTINUE.equals(phase);}
...
void baselineManualResume(int cursor,DriveSignalParser.Event latest,DriveSignalParser.Event latestUnseenCompletion){SharedPreferences.Editor e=clearCommandWait(prefs.edit()).putInt("driveSignalCursor",Math.max(0,cursor)).putInt("driveSignalCursorSchemaVersion",DRIVE_SIGNAL_CURSOR_SCHEMA_PHYSICAL).putBoolean("paused",false).putBoolean("active",true).putBoolean("userStopped",false).putString("phase",PHASE_SEND_CONTINUE).putString("status","사용자 재개 override · CONTINUE 강제 제출 준비").putLong("phaseStartedAt",System.currentTimeMillis());if(latestUnseenCompletion!=null&&latestUnseenCompletion.protocolError.isEmpty()){e.putString("pendingDriveSignalRaw",latestUnseenCompletion.raw).putString("pendingDriveSignalTimestamp",latestUnseenCompletion.timestamp).putString("pendingDriveSignalType",DriveSignalParser.Type.TURN_COMPLETED.name());}else{e.putString("pendingDriveSignalRaw","").putString("pendingDriveSignalTimestamp","").putString("pendingDriveSignalType","");}putLatest(e,latest);commitOrThrow(e);syncHistory();}
...
private void migrateLegacyProtocolTurnState(){synchronized(RUN_STATE_LOCK){String legacyToken=prefs.getString("turnObserverToken","");String token=get("turnProtocolToken");String current=phase();SharedPreferences.Editor e=prefs.edit();boolean changed=false;if(token.isEmpty()&&!legacyToken.isEmpty()){e.putString("turnProtocolToken",legacyToken).putBoolean("turnProtocolCompletionConsumed",false);changed=true;}if("POST_DOM_DRIVE_SYNC".equals(current)){e.putString("phase",PHASE_POST_PROTOCOL_DRIVE_SYNC).putLong("postProtocolDriveSyncStartedAt",prefs.getLong("postDomDriveSyncStartedAt",phaseStartedAt()));changed=true;}if(prefs.contains("turnObserverToken")||prefs.contains("postDomDriveSyncStartedAt")){e.remove("turnObserverToken").remove("postDomDriveSyncStartedAt");changed=true;}if(changed){commitOrThrow(e);syncHistory();}}}
private void migrateLegacyTurnCompletionFlow(){synchronized(RUN_STATE_LOCK){String current=phase();boolean legacy=LEGACY_PHASE_WAIT_DRIVE_COMMIT.equals(current)||PHASE_WAIT_INTERNAL_SEND.equals(current)||current.startsWith("WATCHDOG_");if(!legacy)return;commitOrThrow(clearWatchdogClaimFields(prefs.edit()).putString("phase",PHASE_WAIT_TURN_COMPLETION).putString("turnProtocolToken","").putBoolean("turnProtocolCompletionConsumed",false).putString("turnProtocolCompletionSource","").putLong("postProtocolDriveSyncStartedAt",0L).putString("status","업데이트된 실행 · 응답 프로토콜 재연결 대기").putLong("phaseStartedAt",System.currentTimeMillis()));syncHistory();}}
