...
final class WebUiCalibrationDom {
    ...
    static String install(String purpose) {
        return "(()=>{"
                ...
                + "const inputLike=e=>!!e&&(e.id==='prompt-textarea'||e.matches?.('textarea,[contenteditable=\"true\"]'))&&!!(e.closest?.('form')||e.closest?.('main'));"
                + "const layoutFamily=()=>{const w=window.innerWidth||0,h=window.innerHeight||0;return w>=320&&w<=600&&h>w?"
                + SelfRunScript.quote(AutomationViewportPolicy.MOBILE_LAYOUT_FAMILY) + ":'other_v1';};"
                + "const desc=e=>{if(!e)return null;const p=e.parentElement;return {tag:(e.tagName||'').toLowerCase(),id:norm(e.id),role:norm(e.getAttribute?.('role')),testid:norm(e.dataset?.testid),aria:norm(e.getAttribute?.('aria-label')),name:norm(e.getAttribute?.('name')),type:norm(e.getAttribute?.('type')),text:inputLike(e)?'':norm(e.innerText||e.textContent),href:norm(e.getAttribute?.('href')),parentRole:norm(p?.getAttribute?.('role')),parentTestid:norm(p?.dataset?.testid),parentAria:norm(p?.getAttribute?.('aria-label')),layoutFamily:layoutFamily()};};"
     ...
     * Calibration captured outside a phone-portrait CSS layout is never auto-translated.
     */
    static String runtimePrelude() {
        return "const __srNorm=s=>String(s??'').replace(/\\s+/g,' ').trim().toLowerCase().slice(0,120);"
                ...
                + "const __srLayoutFamily=" + SelfRunScript.quote(AutomationViewportPolicy.MOBILE_LAYOUT_FAMILY) + ";const __srMobileDescriptor=d=>!!d&&d.layoutFamily===__srLayoutFamily;"
                ...
                + "const __srComposerKey=k=>k==='GENERAL_COMPOSER'||k==='PROJECT_COMPOSER';const __srSendKey=k=>k==='GENERAL_SEND'||k==='PROJECT_SEND';const __srNewChatKey=k=>k==='GENERAL_NEW_CHAT'||k==='PROJECT_NEW_CHAT';"
                + "const __srComposerCandidate=e=>!!e&&__srVisible(e)&&(e.id==='prompt-textarea'||e.matches?.('textarea,[contenteditable=\"true\"]'))&&!!(e.closest?.('form')||e.closest?.('main'));"
                + "const __srCurrentComposer=()=>{const selectors=['textarea#prompt-textarea','textarea[data-testid=\"prompt-textarea\"]','div#prompt-textarea[contenteditable=\"true\"]','main form [contenteditable=\"true\"][data-lexical-editor=\"true\"]','main form [contenteditable=\"true\"]'];for(const s of selectors){const e=[...document.querySelectorAll(s)].find(__srVisible);if(e)return e;}return null;};"
                ...
                + "const __srNewChatCandidate=e=>{if(!e||!__srVisible(e)||!e.matches?.('button,a,[role=\"button\"]')||e.closest?.('[role=\"menu\"],[role=\"listbox\"]'))return false;const role=__srNorm(e.getAttribute?.('role')),text=__srNorm(e.innerText||e.textContent),aria=__srNorm(e.getAttribute?.('aria-label')),newChat=/^(new chat|new conversation|새 채팅|새 대화)$/;return!/^(menuitemradio|radio|tab|option)$/.test(role)&&(newChat.test(text)||newChat.test(aria));};"
                + "const __srCompatible=(k,e)=>__srComposerKey(k)?__srComposerCandidate(e):(__srSendKey(k)?__srSendCandidate(e):(__srNewChatKey(k)?__srNewChatCandidate(e):true));"
                ...
                + "const __srFind=k=>{const d=__srTargets[k];if(!d)return null;if(!__srMobileDescriptor(d)){__srTrace(k,'RECALIBRATE','layout='+String(d.layoutFamily||'legacy'));return null;}const nodes=[...document.querySelectorAll('button,a,input,textarea,[contenteditable=\"true\"],[role=\"button\"],[role=\"radio\"],[role=\"tab\"],[role=\"menuitem\"],[role=\"menuitemradio\"],[role=\"option\"]')].filter(__srVisible);let rawBest=null,rawScore=-999,best=null,score=-999;for(const e of nodes){const n=__srScore(e,d);if(n>rawScore){rawScore=n;rawBest=e;}if(__srCompatible(k,e)&&n>score){score=n;best=e;}}const found=score>=6?best:null,event=found?'MATCH':(rawScore>=6?'REJECT':'MISS'),kind=__srComposerKey(k)?'composer':(__srSendKey(k)?'send':(__srNewChatKey(k)?'new-chat':'generic'));__srTrace(k,event,'score='+score+';raw='+rawScore+';kind='+kind);return found;};";
    }
}
