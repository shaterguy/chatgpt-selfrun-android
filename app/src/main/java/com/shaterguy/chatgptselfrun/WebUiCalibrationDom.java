...
final class WebUiCalibrationDom {
    ...
    static String install(String purpose) {
        return "(()=>{"
                ...
                + "const actionable=t=>t?.closest?.('button,a,input,textarea,[contenteditable],[role=\"textbox\"],[role=\"button\"],[role=\"radio\"],[role=\"tab\"],[role=\"menuitem\"],[role=\"menuitemradio\"],[role=\"option\"]')||t;"
                + "const inputLike=e=>{if(!e||!(e.closest?.('form')||e.closest?.('main')||e.closest?.('[data-type=\"unified-composer\"]')||e.closest?.('[class*=\"composer\"]')))return false;const specific=e.id==='prompt-textarea'||e.dataset?.testid==='prompt-textarea'||e.matches?.('textarea');const semantic=editable(e)&&(e.matches?.('[role=\"textbox\"],.ProseMirror,[data-lexical-editor=\"true\"],[contenteditable]'));return !!(specific||semantic);};"
                + "const desc=e=>{if(!e)return null;const p=e.parentElement;return {tag:(e.tagName||'').toLowerCase(),id:norm(e.id),role:norm(e.getAttribute?.('role')),testid:norm(e.dataset?.testid),aria:norm(e.getAttribute?.('aria-label')),name:norm(e.getAttribute?.('name')),type:norm(e.getAttribute?.('type')),text:inputLike(e)?'':norm(e.innerText||e.textContent),href:norm(e.getAttribute?.('href')),parentRole:norm(p?.getAttribute?.('role')),parentTestid:norm(p?.dataset?.testid),parentAria:norm(p?.getAttribute?.('aria-label'))};};"
                ...
                + "const isComposer=e=>inputLike(e);"
     ...
     * Composer/send targets additionally have to match their runtime semantic role so a descriptor
     * captured on a new-chat page cannot shadow the safe semantic fallback in an existing chat.
     */
    static String runtimePrelude() {
        return "const __srNorm=s=>String(s??'').replace(/\\s+/g,' ').trim().toLowerCase().slice(0,120);"
                ...
                + "const __srEditable=e=>{if(!e)return false;const v=__srNorm(e.getAttribute?.('contenteditable'));return !!e.isContentEditable||(e.hasAttribute?.('contenteditable')&&v!=='false');};"
                + "const __srComposerKey=k=>k==='GENERAL_COMPOSER'||k==='PROJECT_COMPOSER';const __srSendKey=k=>k==='GENERAL_SEND'||k==='PROJECT_SEND';"
                + "const __srComposerScope=e=>!!e&&(!!e.closest?.('form')||!!e.closest?.('main')||!!e.closest?.('[data-type=\"unified-composer\"]')||!!e.closest?.('[class*=\"composer\"]'));"
                + "const __srComposerCandidate=e=>{if(!e||!__srVisible(e)||!__srComposerScope(e))return false;const specific=e.id==='prompt-textarea'||e.dataset?.testid==='prompt-textarea'||e.matches?.('textarea');const semantic=__srEditable(e)&&(e.matches?.('[role=\"textbox\"],.ProseMirror,[data-lexical-editor=\"true\"],[contenteditable]'));return !!(specific||semantic);};"
                + "const __srCurrentComposer=()=>{const pool=[...document.querySelectorAll('#prompt-textarea,[data-testid=\"prompt-textarea\"],textarea,[contenteditable],[role=\"textbox\"]')];return pool.find(__srComposerCandidate)||null;};"
                + "const __srSendCandidate=e=>{if(!e||!__srVisible(e))return false;const button=e.matches?.('button,[role=\"button\"]');if(!button)return false;const composer=__srCurrentComposer(),form=composer?.closest?.('form')||null;return !form||form.contains(e);};"
                + "const __srCompatible=(k,e)=>__srComposerKey(k)?__srComposerCandidate(e):(__srSendKey(k)?__srSendCandidate(e):true);"
                ...
                + "const __srFind=k=>{const d=__srTargets[k],semantic=__srComposerKey(k)?__srCurrentComposer():null;if(!d){if(semantic)__srTrace(k,'SEMANTIC','source=current;profile=0');return semantic;}const nodes=[...document.querySelectorAll('button,a,input,textarea,#prompt-textarea,[data-testid=\"prompt-textarea\"],[contenteditable],[role=\"textbox\"],[role=\"button\"],[role=\"radio\"],[role=\"tab\"],[role=\"menuitem\"],[role=\"menuitemradio\"],[role=\"option\"]')].filter(__srVisible);let rawBest=null,rawScore=-999,best=null,score=-999;for(const e of nodes){const n=__srScore(e,d);if(n>rawScore){rawScore=n;rawBest=e;}if(__srCompatible(k,e)&&n>score){score=n;best=e;}}const found=score>=6?best:null;if(found){__srTrace(k,'MATCH','score='+score+';raw='+rawScore+';kind='+(__srComposerKey(k)?'composer':(__srSendKey(k)?'send':'generic')));return found;}if(semantic){__srTrace(k,'SEMANTIC','score='+score+';raw='+rawScore+';profile=1');return semantic;}const event=rawScore>=6?'REJECT':'MISS',kind=__srComposerKey(k)?'composer':(__srSendKey(k)?'send':'generic');__srTrace(k,event,'score='+score+';raw='+rawScore+';kind='+kind);return null;};";
    }
}
