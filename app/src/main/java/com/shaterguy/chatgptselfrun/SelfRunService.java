...
private void guardElapsed(){if(canRun()&&SelfRunStore.PHASE_DRIVE_COMMIT_GUARD.equals(store.phase())){String next=SelfRunStore.MODE_WORK.equals(store.mode())?SelfRunStore.PHASE_APPLY_PREFS:SelfRunStore.PHASE_SEND_CONTINUE;transition(next,"Drive TURN_COMPLETED 확인 · continuation 준비","guard_elapsed");ensureWebView();}}

private void ensureWebView(){if(!canRun()||!isWebAutomationPhase(store.phase()))return;String target=store.conversationUrl().isEmpty()?store.projectUrl():store.conversationUrl();if(target.isEmpty()||!validAutomationTarget(target)){store.setLastError("TARGET_MISSING_RETRY","ChatGPT 대상 URL을 안전하게 재확인합니다.");runLog.record(store,"WEBVIEW_LAUNCH_RETRY",SelfRunWebDiagnostics.targetRetryDetail(store.phase(),SUBMISSION_RETRY_MS));handler.postDelayed(this::ensureWebView,SUBMISSION_RETRY_MS);return;}acquireWakeLock();if(webView!=null){maybeCaptureConversationUrl(webView.getUrl());scheduleWeb(250L);return;}launchWebView(target);}

    private void launchWebView(String target) {
        cleanupWebView();
        try {
            String launchedRunId = store.runId();
            runLog.record(store, "WEBVIEW_LAUNCH", SelfRunWebDiagnostics.launchDetail(store.phase()));
            host = HeadlessWebViewHost.create(this); webView = host.webView(); WebViewConfig.applyAutomation(webView);
            webView.setWebViewClient(new WebViewClient() {
                @Override public void onPageStarted(WebView view, String url, Bitmap favicon) {if (!launchedRunId.equals(store.runId())) return;generation++; domInFlight = false;runLog.record(store,"WEBVIEW_PAGE_START",SelfRunWebDiagnostics.pageDetail(url));maybeCaptureConversationUrl(url);}
                @Override public void onPageFinished(WebView view, String url) {if (!launchedRunId.equals(store.runId())) return;runLog.record(store,"WEBVIEW_PAGE_FINISH",SelfRunWebDiagnostics.pageDetail(url));maybeCaptureConversationUrl(url);if (isWebAutomationPhase(store.phase())) scheduleWeb(800L);}
                @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {if (!launchedRunId.equals(store.runId())) return true;if (!request.isForMainFrame()) return false;String requested = String.valueOf(request.getUrl());boolean allowed = store.conversationUrl().isEmpty()? sameProject(store.projectUrl(), requested) : sameConversation(store.conversationUrl(), requested);if (!allowed) { recordWebRouteMismatch(requested); postWebCallback(SelfRunService.this::restoreCanonical, 800L); }return !allowed;}
                @Override public void onReceivedHttpError(WebView v, WebResourceRequest r, WebResourceResponse s) {if (launchedRunId.equals(store.runId()) && r.isForMainFrame() && s.getStatusCode() == 429){runLog.record(store,"RATE_LIMIT","surface=webview;retry_in_ms=30000");scheduleWeb(30_000L);}}
                @Override public void onReceivedError(WebView v, WebResourceRequest r, WebResourceError e) {if (launchedRunId.equals(store.runId()) && r.isForMainFrame() && canRun() && isWebAutomationPhase(store.phase())){runLog.record(store,"WEBVIEW_ERROR",SelfRunWebDiagnostics.pageErrorDetail(e.getErrorCode(),3_000L));postWebCallback(() -> { if (v == webView) v.loadUrl(canonicalUrl()); }, 3_000L);}}
                ...
                @Override public boolean onRenderProcessGone(WebView v, RenderProcessGoneDetail d) {boolean retry = launchedRunId.equals(store.runId()) && !store.paused() && isWebAutomationPhase(store.phase());if (retry) runLog.record(store,"RENDERER_GONE",SelfRunWebDiagnostics.rendererRetryDetail(2_000L));cleanupWebView();if (retry) postWebCallback(SelfRunService.this::ensureWebView, 2_000L);return true;}
            });
            webView.loadUrl(target);
        } catch (Throwable error) {cleanupWebView();runLog.record(store,"WEBVIEW_LAUNCH_RETRY",SelfRunWebDiagnostics.launchRetryDetail(error,2_500L));postWebCallback(this::ensureWebView, 2_500L);}
    }
...
private void resumeFromUi(){if(!store.paused()||store.userStopped()||store.runId().isEmpty())return;stopAutomationCallbacks();store.beginManualResumeOverride();store.clearLastError();resumeWebView();startForegroundCompat();resumeStateMachine();}
    ...
    private void stopAutomationCallbacks() {removeAutomationCallbacks();synchronized (automationStateLock) {automationEpoch++; generation++; authorizationInFlight = false; domInFlight = false;}}
