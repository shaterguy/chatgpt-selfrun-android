...
private void pollDriveNow(int epoch)throws Exception{
    ...
    if(watchdogPostSubmit){
        if(latestBlocking!=null){if(!applyDriveResult(epoch,()->{store.applyDriveSignals(Collections.singletonList(latestBlocking),System.currentTimeMillis());store.updateDriveSeen(metadata.version,metadata.modifiedTime);} ))return;postDriveOutcome();return;}
        if(!applyDriveResult(epoch,()->{String prior=store.phase();store.finishWatchdogRecoveryBaseline(scan.totalCount,scan.latest);store.updateDriveSeen(metadata.version,metadata.modifiedTime);runLog.record(store,"TURN_COMPLETION_WATCHDOG","post_submit_baseline_cursor="+scan.totalCount+";action=wait_30m");runLog.record(store,"STATE_TRANSITION","from="+prior+";to="+SelfRunStore.PHASE_WAIT_DRIVE_COMMIT+";reason=watchdog_post_submit_baseline");}))return;
...
private String continuationPrompt(){if(continuationAttemptPrompt.isEmpty())continuationAttemptPrompt=isWatchdogRecoverySubmissionPhase(store.phase())?SelfRunProtocol.driveContinuation(store.runId()):SelfRunProtocol.driveContinuation(store.runId(),store.pendingNextInput());return continuationAttemptPrompt;}
...
private void continuationSubmitted(String detail){if(!canRun())return;runLog.record(store,"CONTINUATION_SUBMISSION_CONFIRMED","detail="+detail);boolean watchdog=isWatchdogRecoverySubmissionPhase(store.phase())&&store.watchdogClaimOwned();clearContinuationAttempt();if(watchdog){String prior=store.phase();store.confirmWatchdogSubmission(PHASE_WATCHDOG_POST_SUBMIT_BASELINE,"복구 CONTINUE 제출 확인 · 늦은 이전 completion baseline 정리");runLog.record(store,"STATE_TRANSITION","from="+prior+";to="+PHASE_WATCHDOG_POST_SUBMIT_BASELINE+";reason=watchdog_submission_confirmed");releaseWakeLock();handler.post(this::authorizeAndRunDrive);return;}store.abandonWatchdogClaim();transition(SelfRunStore.PHASE_WAIT_DRIVE_COMMIT,"Drive 턴 결과 신호 대기","continuation_submission_confirmed");releaseWakeLock();scheduleDrivePoll(0L);}
    ...
    private void handleDriveFailure(Throwable error, int epoch) {
        ...
        if (error instanceof DriveApiClient.OutcomeUnknownException) {
            if (PHASE_WATCHDOG_FINAL_RECHECK.equals(store.phase())) {
                ...
                message = "복구 CONTINUE 제출 후 Drive baseline을 다시 확인합니다.";
