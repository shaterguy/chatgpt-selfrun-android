...
private void startConversationSyncNavigation(){if(!canRun()||!SelfRunStore.PHASE_SYNC_CONVERSATION.equals(store.phase())||webView==null||conversationSyncInFlight)return;beginConversationSyncCycle();String canonical=canonicalUrl(),current=webView.getUrl();boolean match=sameConversation(canonical,current);if(match){activeConversationSyncNavigation="reuse";activeConversationSyncGeneration=generation;runLog.record(store,"CONVERSATION_SYNC_NAVIGATION",SelfRunWebDiagnostics.syncDetail(activeConversationSyncEpoch,generation,true,activeConversationSyncNavigation,-1,false,false,true,false));requestConversationVisualReady(webView,activeConversationSyncEpoch,generation);return;}activeConversationSyncNavigation="loadUrl";conversationSyncRecoveryLoadUsed=true;runLog.record(store,"CONVERSATION_SYNC_NAVIGATION",SelfRunWebDiagnostics.syncDetail(activeConversationSyncEpoch,generation,false,activeConversationSyncNavigation,-1,false,false,true,false));webView.loadUrl(canonical);}
