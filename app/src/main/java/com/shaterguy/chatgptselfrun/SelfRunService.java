    ...
    private void executeDriveStep(int epoch) {
        ...
        io.execute(() -> {
            try {
                ...
                if (SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase()) || SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase()) || PHASE_WATCHDOG_RECHECK.equals(store.phase())) {
                    pollDriveNow(epoch);
                    retryAttempt = 0;
                    return;
                }
                do {
                    ...
                    if (SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase())
                            || SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase()) || PHASE_WATCHDOG_RECHECK.equals(store.phase())) break;
...
private void pollDrive(){if((SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase())||SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase())||PHASE_WATCHDOG_RECHECK.equals(store.phase()))&&canRun())authorizeAndRunDrive();}
...
private void postDriveOutcome(){handler.post(()->{String phase=store.phase();if(SelfRunStore.PHASE_WAIT_INTERNAL_SEND.equals(phase)||PHASE_WATCHDOG_SEND_CONTINUE.equals(phase))ensureWebView();else if(SelfRunStore.PHASE_PAUSED.equals(phase)||SelfRunStore.PHASE_DONE.equals(phase)){if(store.terminalSideEffectPending())replayTerminalSideEffect();}else if(SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(phase))scheduleDrivePoll();});}
...
private void pollDriveNow(int epoch)throws Exception{
    ...
    if(!changed&&!resume&&!watchdogRecheck){if(turnCompletionWatchdogDue(snapshot.phase,store.phaseStartedAt(),System.currentTimeMillis()))startTurnCompletionWatchdog(epoch);else applyDriveResult(epoch,this::scheduleDrivePoll);return;}
...
private static String kindForPhase(String phase){return SelfRunStore.PHASE_BOOTSTRAP_SEND.equals(phase)?SelfRunStore.RETRY_BOOTSTRAP:SelfRunStore.RETRY_CONTINUE;}
    ...
    private static boolean isSubmissionPhase(String phase) {return SelfRunStore.PHASE_BOOTSTRAP_SEND.equals(phase) || SelfRunStore.PHASE_SEND_CONTINUE.equals(phase) || PHASE_WATCHDOG_SEND_CONTINUE.equals(phase);}
