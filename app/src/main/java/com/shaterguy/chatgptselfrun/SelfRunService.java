    ...
    private void executeDriveStep(int epoch) {
        ...
        io.execute(() -> {
            try {
                ...
                if (SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase()) || SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase()) || PHASE_WATCHDOG_RECHECK.equals(store.phase()) || PHASE_WATCHDOG_FINAL_RECHECK.equals(store.phase())) {
                    pollDriveNow(epoch);
                    retryAttempt = 0;
                    return;
                }
                do {
                    ...
                    if (SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase())
                            || SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase()) || PHASE_WATCHDOG_RECHECK.equals(store.phase()) || PHASE_WATCHDOG_FINAL_RECHECK.equals(store.phase())) break;
...
private void pollDrive(){if((SelfRunStore.PHASE_WAIT_DRIVE_COMMIT.equals(store.phase())||SelfRunStore.PHASE_RESUME_BASELINE.equals(store.phase())||PHASE_WATCHDOG_RECHECK.equals(store.phase())||PHASE_WATCHDOG_FINAL_RECHECK.equals(store.phase()))&&canRun())authorizeAndRunDrive();}
...
private void pollDriveNow(int epoch)throws Exception{
    ...
    if(!changed&&!resume&&!watchdogRecheck&&!watchdogFinalRecheck){if(turnCompletionWatchdogDue(snapshot.phase,store.phaseStartedAt(),System.currentTimeMillis()))startTurnCompletionWatchdog(epoch);else applyDriveResult(epoch,this::scheduleDrivePoll);return;}
    String text=drive.readDocumentText(accessToken,snapshot.turnDocumentId);if(!canApplyDriveResult(epoch))return;
    DriveSignalParser.Scan scan=DriveSignalParser.scan(text,snapshot.runId,snapshot.driveSignalCursor,snapshot.mode);
    ...
    if(watchdogRecheck){
        if(!applyDriveResult(epoch,()->{if(scan.cursorRebased)store.baselineDriveSignals(scan.totalCount,scan.latest);else if(!scan.unseen.isEmpty())store.applyDriveSignals(scan.unseen,System.currentTimeMillis());store.updateDriveSeen(metadata.version,metadata.modifiedTime);if(PHASE_WATCHDOG_RECHECK.equals(store.phase())){runLog.record(store,"TURN_COMPLETION_WATCHDOG","drive_recheck=no_completion;action=prepare_plain_continue");transition(PHASE_WATCHDOG_SEND_CONTINUE,"Drive 재확인 완료 · 복구 CONTINUE 입력 준비","watchdog_drive_recheck_clear");}}))return;
        postDriveOutcome();return;
    }
    if(watchdogFinalRecheck){
        if(!applyDriveResult(epoch,()->{if(scan.cursorRebased)store.baselineDriveSignals(scan.totalCount,scan.latest);else if(!scan.unseen.isEmpty())store.applyDriveSignals(scan.unseen,System.currentTimeMillis());store.updateDriveSeen(metadata.version,metadata.modifiedTime);if(PHASE_WATCHDOG_FINAL_RECHECK.equals(store.phase())){runLog.record(store,"TURN_COMPLETION_WATCHDOG","drive_final_recheck=no_completion;action=click_prepared_continue");transition(PHASE_WATCHDOG_CLICK_CONTINUE,"Drive 최종 재확인 완료 · 준비된 복구 CONTINUE 제출","watchdog_final_drive_fence_clear");}}))return;
        if(!PHASE_WATCHDOG_CLICK_CONTINUE.equals(store.phase()))handler.post(this::clearContinuationAttempt);
        postDriveOutcome();return;
    }
...
private String continuationMarkerId(){if(continuationAttemptMarkerId.isEmpty())continuationAttemptMarkerId=isWatchdogRecoverySubmissionPhase(store.phase())?store.runId()+":watchdog-continue:"+store.driveSignalCursor():store.runId()+":continue:"+store.driveSignalCursor()+":"+store.phaseStartedAt();return continuationAttemptMarkerId;}
private void clearContinuationAttempt(){continuationAttemptPrompt="";continuationAttemptMarkerId="";}
private void continuationSubmitted(String detail){if(!canRun())return;runLog.record(store,"CONTINUATION_SUBMISSION_CONFIRMED","detail="+detail);clearContinuationAttempt();transition(SelfRunStore.PHASE_WAIT_DRIVE_COMMIT,"Drive 턴 결과 신호 대기","continuation_submission_confirmed");releaseWakeLock();scheduleDrivePoll(0L);}
