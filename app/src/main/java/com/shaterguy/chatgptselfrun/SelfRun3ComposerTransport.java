...
/** Initial-message-only composer transport. Dispatch identity is confirmed by the native request bridge. */
final class SelfRun3ComposerTransport {
    ...
    static String prepareBootstrap(String projectUrl, String prompt, String markerId) {
        ...
        return "(() =>{const result=(status,detail='')=>JSON.stringify({status,detail,url:location.href});"
                + projectGuard(project) + freshGuard() + authGuard() + calibration() + textHelpers(expected) + markerOps(marker)
                + "let m=readMarker();"
                + "if(m.state==='clicked'||m.state==='confirmed')return result('SUBMISSION_PENDING','dispatch already claimed');"
    ...
    static String clickPreparedBootstrap(String projectUrl, String prompt, String markerId) {
        ...
        String sendKey = sendKey(projectUrl);
        return "(() =>{const result=(status,detail='')=>JSON.stringify({status,detail,url:location.href});"
                + projectGuard(project) + freshGuard() + authGuard() + calibration() + textHelpers(expected)
    ...
    private static String freshGuard() {
        return "if(after('c'))return result('TARGET_ERROR','existing conversation cannot receive app input');";
    }
}
