...
final class WorkPreferenceDom {
    ...
    private static String preference(String guard, String kind, String wanted, String calibrationPurpose) {
        String body = """
                ...
                const __wpModel=source=>{const v=__wpText(source);const match=v.match(/(?:^|\\s)(?:gpt-?)?5(?:\\.6)?\\s+(sol|terra|luna)(?:\\s|>|$)/);return match?match[1]:'';};
                ...
                if(__wpKind==='model'){
                  if(__wpModelOptions.length>0){
                    if(__wpWantedOption&&__wpSelected(__wpWantedOption.element)){__wpState.lastAction='close-model-menu';__wpState.lastActionAt=__wpNow;__wpSave();__wpEscape();return result('UI_WAIT','WORK 모델 선택 확인 후 설정 팝업 복귀 대기',__wpDiagnostics({action:'close-model-menu'}));}
                  ...
                  if(__wpReasoningOptions.length>0&&!__wpSlider){
                    if(__wpWantedOption&&__wpSelected(__wpWantedOption.element)){__wpState.verified=__wpWanted;__wpSave();__wpEscape();return result('UI_WAIT','WORK 추론 옵션 선택 확인 후 메뉴 닫힘 대기',__wpDiagnostics({action:'close-reasoning-menu'}));}
