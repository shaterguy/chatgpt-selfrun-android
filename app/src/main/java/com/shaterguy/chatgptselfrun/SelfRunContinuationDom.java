...
final class SelfRunContinuationDom {
    ...
    private static String completionObserver(String runId, String observerToken, long stabilityMs) {
        ...
        return "const observerToken=" + q(observerToken) + ",observerRun=" + q(runId)
                ...
                + "if(!state){state={token:observerToken,sawStop:false,stopNotified:false,allowIdleBaseline:false,timer:0,fired:false,observer:null,root:null,composer:null};window.__selfRunDriveTurnObserver=state;}"
                ...
                + "state.evaluate();"
                + "return result('OBSERVER_ARMED','STOP/SEND observer armed;stableMs='+observerStableMs+';baseline='+(state.allowIdleBaseline?1:0)+';bindingChanged='+(bindingChanged?1:0));};";
    }
