...
final class SelfRunDom {
    ...
    static String prepareInitialContext(String projectUrl, String mode, String runId) {
        ...
        return "(() =>{const result=(status,detail='',diagnostics={})=>JSON.stringify({status,detail,url:location.href,diagnostics});"
                ...
                + "if(actualConversation){if(newChatControl&&!recentNewChat){try{sessionStorage.setItem(newChatKey,JSON.stringify({at:Date.now()}));}catch(_){}newChatControl.focus?.();newChatControl.click();return result('UI_WAIT','보정된 새 대화 전환 반영 대기',{actualConversation,newChatSource:__srFind(" + q(newChatTarget) + ")?'calibrated':'heuristic'});}return result('CHAT_BOOTSTRAP_NEW_CHAT_FAILED','새 대화 화면 대신 기존 conversation이 열렸습니다.',{actualConversation,newChatControl:!!newChatControl,recentNewChat});}"
                ...
                + "const modeKey='chatgpt-selfrun:mode:" + esc(runId) + "';const modeTimeoutMs=20000,modeMaxAttempts=18;let modeState={startedAt:0,attempts:0,clickAttempts:0,lastClickAt:0,lastAction:'',requested:''};try{const raw=sessionStorage.getItem(modeKey)||localStorage.getItem(modeKey)||'';if(raw)modeState={...modeState,...JSON.parse(raw)};}catch(_){}const modeNow=Date.now();if(!(Number(modeState.startedAt)>0))modeState.startedAt=modeNow;modeState.attempts=Math.max(0,Number(modeState.attempts)||0)+1;modeState.requested=requestedMode;const modeElapsedMs=Math.max(0,modeNow-Number(modeState.startedAt||modeNow));const saveMode=()=>{const value=JSON.stringify(modeState);try{sessionStorage.setItem(modeKey,value);}catch(_){}try{localStorage.setItem(modeKey,value);}catch(_){}};const clearMode=()=>{try{sessionStorage.removeItem(modeKey);}catch(_){}try{localStorage.removeItem(modeKey);}catch(_){}};const recentClick=Number(modeState.lastClickAt)>0&&modeNow-Number(modeState.lastClickAt)<1200;"
      + "let action='';const calibratedImplicit=modeState.lastAction==='select-mode-calibrated'&&modeState.requested===requestedMode&&Number(modeState.lastClickAt)>0&&modeNow-Number(modeState.lastClickAt)<5000&&!!composer;const heuristicReadback=targetFound&&targetSelected&&currentMode===requestedMode&&selectedModes.length===1;let modeReadback=calibratedImplicit||(targetSource==='calibrated'?targetSelected:heuristicReadback);if(!modeReadback&&targetFound&&!recentClick&&Number(modeState.clickAttempts)<2){action=targetSource==='calibrated'?'select-mode-calibrated':'select-mode';modeState.clickAttempts=Math.max(0,Number(modeState.clickAttempts)||0)+1;modeState.lastClickAt=modeNow;modeState.lastAction=action;saveMode();target.focus?.();target.click();modeReadback=false;}"
      + "const diagnostics={requested:requestedMode,currentMode,modeCandidates:rawModeControls.length,groupFound:!!modeGroup,targetFound,targetSelected,targetSource,selectedModes,recentClick,action,calibratedImplicit,composer:!!composer,finalReadback:modeReadback,modeAttempts:modeState.attempts,modeClickAttempts:modeState.clickAttempts,modeElapsedMs,modeTimeoutMs};const modeDiag=()=>('requested='+requestedMode+';source='+targetSource+';current='+currentMode+';targetFound='+(targetFound?1:0)+';targetSelected='+(targetSelected?1:0)+';attempt='+(action||'none')+';readback='+(modeReadback?1:0)+';elapsedMs='+modeElapsedMs);"
      + "if(action)return result('UI_WAIT','모드 전환 반영 대기 · '+modeDiag(),diagnostics);if(!modeReadback){saveMode();if(!targetFound&&(modeElapsedMs>=modeTimeoutMs||modeState.attempts>=modeMaxAttempts))return result('CHAT_BOOTSTRAP_MODE_CONTROL_NOT_FOUND','실행 모드 선택기를 제한시간 안에 찾지 못했습니다.',diagnostics);if(targetFound&&modeState.clickAttempts>=2&&(modeElapsedMs>=4800||modeState.attempts>=8))return result('CHAT_BOOTSTRAP_MODE_READBACK_FAILED','실행 모드 선택 후 실제 상태를 확인하지 못했습니다.',diagnostics);if(modeElapsedMs>=modeTimeoutMs||modeState.attempts>=modeMaxAttempts)return result('CHAT_BOOTSTRAP_MODE_READBACK_FAILED','실행 모드 실제 상태 확인이 제한시간을 초과했습니다.',diagnostics);return result('UI_WAIT','실행 모드 실제 상태 대기 · '+modeDiag(),diagnostics);}"
      + "if(!composer){saveMode();if(modeElapsedMs>=modeTimeoutMs||modeState.attempts>=modeMaxAttempts)return result('CHAT_BOOTSTRAP_COMPOSER_NOT_FOUND','새 대화 입력창을 제한시간 안에 찾지 못했습니다.',diagnostics);return result('UI_WAIT','새 대화 입력창 대기 · '+modeDiag(),diagnostics);}clearMode();"
