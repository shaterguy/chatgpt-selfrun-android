...
final class SelfRunDom {
    ...
    private static String composer(String targetKey) {
        return "const __srComposerSelector='textarea#prompt-textarea,textarea[data-testid=\"prompt-textarea\"],div#prompt-textarea[contenteditable=\"true\"],main form [contenteditable=\"true\"][data-lexical-editor=\"true\"],main form [contenteditable=\"true\"]';const __srComposerPool=()=>{const all=[...document.querySelectorAll(__srComposerSelector)].filter(e=>e&&e.isConnected&&e.offsetParent!==null);const outsideTurns=all.filter(e=>!e.closest('[data-message-author-role],[data-testid^=\"conversation-turn\"],article[data-testid^=\"conversation-turn\"]'));return outsideTurns.length?outsideTurns:all;};const __srLatestComposer=()=>{const xs=__srComposerPool();return xs.length?xs[xs.length-1]:null;};const calibratedComposer=__srFind(" + q(targetKey) + ");let composer=__srLatestComposer()||calibratedComposer;";
    }
