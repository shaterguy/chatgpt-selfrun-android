...
final class SelfRunDom {
    ...
    static String prepareInitialContext(String projectUrl, String mode, String runId) {
        ...
        String profileSetup = BootstrapModeDom.inline(requested, runId)
                + (work ? ChatReasoningOptionDom.inline(chatReasoning, runId)
                : "return result('READY','CHAT profile deferred to current turn profile stage',{strategy:'request-profile',action:'skip-chat-profile-initial'});");
    ...
    private static String projectGuard(String project) {
        ...
        return "if(location.hostname!=='chatgpt.com'&&location.hostname!=='www.chatgpt.com')return result('TARGET_ERROR','호스트 불일치');"
                ...
                + "const p=location.pathname.split('/').filter(Boolean);const afterProject=k=>{const i=p.indexOf(k);return i>=0&&i+1<p.length?p[i+1]:''};const expectedProject=" + project + ";const actualProject=__srCanonicalProjectId(afterProject('g'));"
                + "if(expectedProject===" + general + "){const generalNew=p.length===0;const generalConversation=p.length===2&&p[0]==='c'&&!!p[1];if(!generalNew&&!generalConversation)return result('TARGET_ERROR','일반 Chat 범위 이탈');}else if(!actualProject||actualProject!==expectedProject)return result('TARGET_ERROR','프로젝트 불일치');";
    }
