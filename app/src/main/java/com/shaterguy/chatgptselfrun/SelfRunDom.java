...
static String prepareDriveTurn(String conversationUrl,String prompt,String markerId){String conversation=q(SelfRunScript.conversationId(conversationUrl)),expected=q(prompt),marker=q("selfrun-drive:command:"+markerId);return "(() =>{const result=(status,detail='',extra={})=>JSON.stringify({status,detail,url:location.href,...extra});"+conversationGuard(conversation)+authGuard()+textHelpers(expected)+parentGuardOutcome(markerId)+composer()+"if(!composer)return result('UI_WAIT','continuation 입력창 대기',{stage:'COMPOSER_WAIT'});"+composerOps()+"if(same()){const send=findSend();if(!send||send.disabled||send.getAttribute('aria-disabled')==='true')return result('UI_WAIT','continuation 전송 버튼 대기',{stage:'SEND_BUTTON_WAIT'});const markerKey2="+marker+",v=JSON.stringify({state:'prepared',at:Date.now()});let persisted=false;try{localStorage.setItem(markerKey2,v);persisted=localStorage.getItem(markerKey2)===v;}catch(_){}if(!persisted){try{sessionStorage.setItem(markerKey2,v);persisted=sessionStorage.getItem(markerKey2)===v;}catch(_){}}if(!persisted)return result('MARKER_FAILED','continuation 제출 표식 저장 실패');return result('READY_TO_SUBMIT','continuation 제출 준비 완료',{stage:'READY_TO_SUBMIT'});}"+input()+"return result('UI_WAIT',same()?'입력 반영 확인 대기':'continuation 입력 대기',{stage:'COMPOSER_INPUT_WAIT'});})()";}
...
static String readLatestSelfRunControl(String conversationUrl,String runId){String conversation=q(SelfRunScript.conversationId(conversationUrl)),run=q(runId);return "(() =>{const result=(status,signal='')=>JSON.stringify({status,signal,url:location.href});"+conversationGuard(conversation)+authGuard()+"const run="+run+";const esc=s=>String(s).replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&');const re=new RegExp('\\\\[SELF_RUN_NEXT\\\\s+'+esc(run)+'\\\\s+ROLE=[A-Za-z0-9._:-]+(?:\\\\s+MODEL=[A-Za-z]+\\\\s+REASONING=[A-Za-z]+)?\\\\]','g');const nodes=[...document.querySelectorAll('[data-message-author-role=\\\"assistant\\\"],article[data-turn=\\\"assistant\\\"]')];let last='';for(const node of nodes){const text=String(node.innerText||node.textContent||'');const found=text.match(re);if(found&&found.length)last=found[found.length-1];}return last?result('CONTROL_FOUND',last):result('CONTROL_MISSING','');})()";}
    ...
    private static String parentGuardOutcome(String markerId) {
        ...
        String memoryResult = SelfRunNetworkGuard.MEMORY_RESULT;
        String livenessFn = SelfRunNetworkGuard.LIVENESS_FN;
        return "const srMarkerId=" + marker + ",srArmKey=" + armKey + ",srResultKey=" + resultKey + ";"
                + "const srSafeCode=c=>{c=String(c||'');const fixed=['HOOK_UNAVAILABLE','HOOK_LOST','ARM_STORAGE_FAILED','NO_POST_AFTER_CLICK','ENDPOINT_MISMATCH','PAYLOAD_MISMATCH','PARENT_ID_MISSING','BODY_UNREADABLE','CANONICAL_MISSING','CANONICAL_GENERATING','HANDSHAKE_TIMEOUT','FORWARD_FAILED','GUARD_INTERNAL_FAILURE'];return fixed.includes(c)||/^CANONICAL_HTTP_[1-5][0-9]{2}$/.test(c)?c:'GUARD_INTERNAL_FAILURE';};"
                + "const srStage=s=>({post_intercepted:'POST_INTERCEPTED',payload_matched:'PAYLOAD_MATCHED',canonical_fetch_start:'CANONICAL_FETCH_START',canonical_fetch_ok:'CANONICAL_FETCH_OK',parent_rewritten:'PARENT_REWRITTEN',forwarding:'FORWARDING'})[String(s||'')]||'POST_INTERCEPTION_WAIT';"
                + "const srClear=()=>{try{sessionStorage.removeItem(srResultKey);sessionStorage.removeItem(srArmKey);}catch(_){}try{window." + memoryResult + "=null;}catch(_){}};"
                + "let srGuardResult=null,srGuardArm=null;try{const mem=window." + memoryResult + ";if(mem&&String(mem.markerId||'')===srMarkerId)srGuardResult=mem;}catch(_){}if(!srGuardResult){try{const raw=sessionStorage.getItem(srResultKey)||'';srGuardResult=raw?JSON.parse(raw):null;}catch(_){}}try{const raw=sessionStorage.getItem(srArmKey)||'';srGuardArm=raw?JSON.parse(raw):null;}catch(_){}"
                + "if(srGuardResult&&String(srGuardResult.markerId||'')===srMarkerId&&srGuardResult.state==='forwarded'){srClear();return result('SUBMITTED','canonical parent guard 제출 확인',{stage:'SUBMISSION_CONFIRMED',guardCode:'CANONICAL_PARENT_APPLIED'});}"
                + "if(srGuardResult&&String(srGuardResult.markerId||'')===srMarkerId&&srGuardResult.state==='failed'){const c=srSafeCode(srGuardResult.code);srClear();return result('PARENT_GUARD_FAILED','canonical parent guard 실패',{stage:'FAILED',guardCode:c});}"
                + "if(srGuardArm&&String(srGuardArm.markerId||'')===srMarkerId){let alive=false;try{alive=window." + SelfRunNetworkGuard.INSTALLED_FLAG + "===true&&typeof window." + livenessFn + "==='function'&&window." + livenessFn + "()===true;}catch(_){}if(!alive){srClear();return result('PARENT_GUARD_FAILED','canonical parent guard hook liveness 손실',{stage:'HOOK_UNAVAILABLE',guardCode:'HOOK_LOST'});}const expires=Number(srGuardArm.expiresAt||0);if(expires>0&&expires<=Date.now()){const observed=!!(srGuardResult&&String(srGuardResult.markerId||'')===srMarkerId);srClear();return result('PARENT_GUARD_FAILED','canonical parent guard 제한시간 초과',{stage:'TIMEOUT',guardCode:observed?'HANDSHAKE_TIMEOUT':'NO_POST_AFTER_CLICK'});}if(srGuardResult&&String(srGuardResult.markerId||'')===srMarkerId)return result('UI_WAIT','canonical parent guard 처리 중',{stage:srStage(srGuardResult.state)});return result('UI_WAIT','canonical parent guard 네트워크 제출 대기',{stage:'POST_INTERCEPTION_WAIT'});}";
    }
    ...
    private static String armParentGuard(String conversationId, String prompt, String markerId) {
        ...
        String memoryResult = SelfRunNetworkGuard.MEMORY_RESULT;
        String livenessFn = SelfRunNetworkGuard.LIVENESS_FN;
        return "if(window." + SelfRunNetworkGuard.INSTALLED_FLAG + "!==true||typeof window." + livenessFn + "!=='function')return result('PARENT_GUARD_FAILED','document-start parent guard 사용 불가',{stage:'HOOK_UNAVAILABLE',guardCode:'HOOK_UNAVAILABLE'});"
                + "let srAlive=false;try{srAlive=window." + livenessFn + "()===true;}catch(_){}if(!srAlive)return result('PARENT_GUARD_FAILED','document-start parent guard liveness 손실',{stage:'HOOK_UNAVAILABLE',guardCode:'HOOK_LOST'});"
                + "try{sessionStorage.removeItem(" + resultKey + ");window." + memoryResult + "=null;sessionStorage.setItem(" + armKey + ",JSON.stringify({markerId:" + marker + ",conversationId:" + conversation + ",expected:" + expected + ",armedAt:Date.now(),expiresAt:Date.now()+30000}));}catch(_){return result('PARENT_GUARD_FAILED','canonical parent guard 저장 실패',{stage:'ARM_FAILED',guardCode:'ARM_STORAGE_FAILED'});}";
    }
