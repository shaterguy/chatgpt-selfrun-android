...
final class SelfRunDom {
    ...
    static String prepareInitialContext(String projectUrl, String mode, String runId) {
        ...
        return "(() =>{const result=(status,detail='',diagnostics={})=>JSON.stringify({status,detail,url:location.href,diagnostics});"
                ...
                + "let action='';const chatDefaultWithoutControls=requestedMode==='chat'&&rawModeControls.length===0;let modeReadback=chatDefaultWithoutControls||(targetFound&&targetSelected&&currentMode===requestedMode&&selectedModes.length===1);if(!modeReadback&&targetFound&&!recentClick){try{sessionStorage.setItem(modeKey,JSON.stringify({at:Date.now(),action:'select-mode',requested:requestedMode}));}catch(_){}target.focus?.();target.click();action='select-mode';modeReadback=false;}"
                + "const diagnostics={requested:requestedMode,currentMode,modeCandidates:rawModeControls.length,groupFound:!!modeGroup,targetFound,targetSelected,selectedModes,recentClick,action,chatDefaultWithoutControls,finalReadback:modeReadback};const modeDiag=()=>('requested='+requestedMode+';current='+currentMode+';targetFound='+(targetFound?1:0)+';targetSelected='+(targetSelected?1:0)+';attempt='+(action||'none')+';readback='+(modeReadback?1:0));"
                ...
                + composer() + "if(!composer)return result('UI_WAIT','ChatGPT 새 대화 입력창 대기 · '+modeDiag(),diagnostics);"
