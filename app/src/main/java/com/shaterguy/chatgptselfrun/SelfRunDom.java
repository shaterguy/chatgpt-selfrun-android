...
final class SelfRunDom {
    ...
    static String prepareInitialContext(String projectUrl, String mode, String runId) {
        ...
        ChatRoutePolicy.Route route = ChatRoutePolicy.parse(projectUrl);
        String expectedNewPath = route != null && route.general
                ? "/" : "/g/" + (route == null ? "" : route.projectId) + "/project";
        ...
        return "(() =>{const result=(status,detail='',diagnostics={})=>JSON.stringify({status,detail,url:location.href,diagnostics});"
                ...
                + "const expectedNewPath=" + q(expectedNewPath) + ";const visible=e=>!!e&&e.isConnected&&e.offsetParent!==null;const exactText=s=>String(s??'').replace(/\\s+/g,' ').trim().toLowerCase();"
                + "const newChatLabel=e=>exactText((e?.innerText||'')+' '+(e?.getAttribute?.('aria-label')||'')+' '+(e?.getAttribute?.('title')||''));const sameScopeNewHref=e=>{const href=e?.getAttribute?.('href')||'';if(!href)return false;try{const u=new URL(href,location.origin);return u.origin===location.origin&&u.pathname===expectedNewPath&&!u.search&&!u.hash}catch(_){return false}};"
                + "const newChatControl=[...document.querySelectorAll('a,button,[role=\"button\"]')].filter(visible).find(e=>{if(e.tagName==='A')return sameScopeNewHref(e);const testId=exactText(e.dataset?.testid||'');if(testId==='create-new-chat-button'||testId==='new-chat-button')return true;return !!e.closest('nav,aside,header')&&/^(new chat|new conversation|새 채팅|새 대화|새로운 대화)$/.test(newChatLabel(e));})||null;"
                + "const newChatKey='chatgpt-selfrun:new-chat:" + esc(runId) + "';let newChatAt=0;try{const raw=sessionStorage.getItem(newChatKey)||'';newChatAt=Number(raw?JSON.parse(raw)?.at||0:0);}catch(_){}const recentNewChat=newChatAt>0&&Date.now()-newChatAt<1500;"
                + "if(actualConversation){if(newChatControl&&!recentNewChat){try{sessionStorage.setItem(newChatKey,JSON.stringify({at:Date.now()}));}catch(_){}newChatControl.focus?.();newChatControl.click();return result('NEW_CONVERSATION_REQUESTED','기존 conversation에서 새 대화 전환 요청',{newChatControl:true});}return result('EXISTING_CONVERSATION','새 대화 화면 대신 기존 conversation이 열렸습니다.',{newChatControl:!!newChatControl,recentNewChat});}"
                + "const turnCount=document.querySelectorAll('[data-message-author-role],article[data-turn]').length;if(turnCount)return result('STALE_NEW_ROUTE','conversation 없는 경로에 기존 턴이 남아 있습니다.',{turnCount});try{sessionStorage.removeItem(newChatKey);}catch(_){}"
                + composer()
                ...
                + "let action='';const chatDefaultWithoutControls=requestedMode==='chat'&&rawModeControls.length===0;const chatImplicitAfterClick=requestedMode==='chat'&&targetFound&&!targetSelected&&selectedModes.length===0&&priorAt>0&&priorAction==='select-mode'&&priorRequested==='chat'&&!!composer;let modeReadback=chatDefaultWithoutControls||(targetFound&&targetSelected&&currentMode===requestedMode&&selectedModes.length===1)||chatImplicitAfterClick;if(!modeReadback&&targetFound&&!recentClick){try{sessionStorage.setItem(modeKey,JSON.stringify({at:Date.now(),action:'select-mode',requested:requestedMode}));}catch(_){}target.focus?.();target.click();action='select-mode';modeReadback=false;}"
                + "const diagnostics={requested:requestedMode,currentMode,modeCandidates:rawModeControls.length,groupFound:!!modeGroup,targetFound,targetSelected,selectedModes,recentClick,action,chatDefaultWithoutControls,chatImplicitAfterClick,finalReadback:modeReadback};const modeDiag=()=>('requested='+requestedMode+';current='+currentMode+';targetFound='+(targetFound?1:0)+';targetSelected='+(targetSelected?1:0)+';attempt='+(action||'none')+';readback='+(modeReadback?1:0));"
                ...
                + "if(!composer)return result('UI_WAIT','ChatGPT 새 대화 입력창 대기 · '+modeDiag(),diagnostics);"
                + "return result('READY','ChatGPT 새 대화 화면 확인 · '+modeDiag(),{...diagnostics,composer:true});})()";
    }
...
static String readLatestSelfRunControl(String conversationUrl,String runId){String conversation=q(SelfRunScript.conversationId(conversationUrl)),run=q(runId);return "(() =>{const result=(status,signal='')=>JSON.stringify({status,signal,url:location.href});"+conversationGuard(conversation)+authGuard()+"const run="+run+";const esc=s=>String(s).replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&');const re=new RegExp('\\\\[SELF_RUN_NEXT\\\\s+'+esc(run)+'\\\\s+ROLE=[A-Za-z0-9._:-]+(?:\\\\s+MODEL=[A-Za-z]+\\\\s+REASONING=[A-Za-z]+)?\\\\]','g');const nodes=[...document.querySelectorAll('[data-message-author-role=\\\"assistant\\\"],article[data-turn=\\\"assistant\\\"]')];let last='';for(const node of nodes){const text=String(node.innerText||node.textContent||'');const found=text.match(re);if(found&&found.length)last=found[found.length-1];}return last?result('CONTROL_FOUND',last):result('CONTROL_MISSING','');})()";}
    ...
    private static String freshRouteGuard(String project) {
        return projectGuard(project)
                + "const freshParts=location.pathname.split('/').filter(Boolean),freshIndex=freshParts.indexOf('c'),freshConversation=freshIndex>=0&&freshIndex+1<freshParts.length?freshParts[freshIndex+1]:'';if(freshConversation)return result('EXISTING_CONVERSATION','bootstrap 제출 전 기존 conversation 감지');"
                + "const freshTurnCount=document.querySelectorAll('[data-message-author-role],article[data-turn]').length;if(freshTurnCount)return result('STALE_NEW_ROUTE','bootstrap 제출 전 기존 턴 감지');";
    }
