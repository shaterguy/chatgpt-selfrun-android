...
final class SelfRunDom {
    ...
    static String prepareInitialContext(String projectUrl, String mode, String runId) {
        ...
        return "(() =>{const result=(status,detail='',diagnostics={})=>JSON.stringify({status,detail,url:location.href,diagnostics});"
                ...
                + "const requestedMode=" + q(requested) + ";if(!composer)return result('UI_WAIT','새 대화 입력창 대기',{requested:requestedMode,composer:false});const forbiddenMode=/new chat|새 채팅|새 대화|new conversation/i;"
                ...
                + "const optionRole=e=>/^(menuitemradio|radio|option|menuitem)$/.test(exactText(e?.getAttribute?.('role')||''));const insideChoice=e=>!!e?.closest?.('[role=\"menu\"],[role=\"listbox\"],[role=\"dialog\"]');const menuTrigger=e=>{if(!e||optionRole(e)||insideChoice(e))return false;const popup=exactText(e.getAttribute?.('aria-haspopup')||'');return/^(menu|listbox|dialog|true)$/.test(popup)||e.getAttribute?.('aria-expanded')!==null||!!e.matches?.('button,[role=\"button\"]')};"
                + "const nearComposer=e=>{if(!composer||!e)return false;const form=composer.closest?.('form')||null;if(form?.contains(e))return true;const a=e.getBoundingClientRect?.(),b=composer.getBoundingClientRect?.();return!!a&&!!b&&a.bottom>=b.top-240&&a.top<=b.bottom+240&&a.right>=b.left-320&&a.left<=b.right+320};"
                + "const desired=e=>e?.getAttribute?.('aria-expanded');const reached=(e,want)=>desired(e)!==null&&desired(e)===String(want);const pointer=e=>{const init={bubbles:true,cancelable:true,composed:true,button:0,buttons:1,pointerId:1,pointerType:'mouse',isPrimary:true};if(typeof PointerEvent!=='function')return false;e.dispatchEvent(new PointerEvent('pointerdown',init));return true};const mouse=e=>e.dispatchEvent(new MouseEvent('mousedown',{bubbles:true,cancelable:true,composed:true,button:0,buttons:1}));const toggleMenu=(e,want)=>{if(!e)return;e.focus?.();const tracked=desired(e)!==null;e.click?.();if(!tracked||reached(e,want))return;if(pointer(e)&&reached(e,want))return;mouse(e)};"
                + "const rawModeControls=[...document.querySelectorAll('button,[role=\"button\"],[role=\"radio\"],[role=\"tab\"],input[type=\"radio\"],[aria-haspopup],[aria-expanded]')].filter(visible).filter(nearComposer).filter(e=>{if(insideChoice(e))return false;const m=modeOf(labelOf(e));if(!m)return false;const role=e.getAttribute('role')||'',testId=exactText(e.dataset?.testid||'');return menuTrigger(e)||e.hasAttribute('aria-pressed')||e.hasAttribute('aria-checked')||e.hasAttribute('aria-selected')||role==='radio'||role==='tab'||e.matches('input[type=\"radio\"]')||/mode|experience/.test(testId);});"
                + "const modeTrigger=rawModeControls.find(menuTrigger)||null;const triggerExpanded=!!modeTrigger&&modeTrigger.getAttribute('aria-expanded')==='true';const triggerCurrent=modeTrigger?modeOf(labelOf(modeTrigger)):'';const visibleOptions=[...document.querySelectorAll('[role=\"menuitemradio\"],[role=\"radio\"],[role=\"option\"],[role=\"menuitem\"]')].filter(visible).filter(e=>!!modeOf(labelOf(e)));"
                + "const calibratedKey=requestedMode==='work'?" + q(WebUiCalibrationStore.PURPOSE_MODE_WORK) + ":" + q(WebUiCalibrationStore.PURPOSE_MODE_CHAT) + ";const rawCalibratedTarget=__srFind(calibratedKey);const calibratedTarget=rawCalibratedTarget&&modeOf(labelOf(rawCalibratedTarget))===requestedMode?rawCalibratedTarget:null;const semanticTarget=visibleOptions.find(e=>modeOf(labelOf(e))===requestedMode)||null;const target=calibratedTarget||semanticTarget;const targetSource=calibratedTarget?'calibrated':(semanticTarget?'semantic':'none');const targetFound=!!target;const targetSelected=selectedState(target);"
                + "const selectedModes=[...new Set(visibleOptions.filter(selectedState).map(e=>modeOf(labelOf(e))).filter(Boolean))];const currentMode=selectedModes.length===1?selectedModes[0]:(selectedModes.length>1?'ambiguous':(triggerCurrent||'unknown'));"
                + "const modeKey='chatgpt-selfrun:mode:" + esc(runId) + "';let priorAt=0;try{const raw=sessionStorage.getItem(modeKey)||'';const parsed=raw?JSON.parse(raw):null;priorAt=Number(parsed?.at||0);}catch(_){}const retryIntervalMs=1200;const recentAction=priorAt>0&&Date.now()-priorAt<retryIntervalMs;let action='';let modeReadback=currentMode===requestedMode;"
                + "if(!modeReadback&&targetFound&&!recentAction){action=targetSource==='calibrated'?'select-mode-calibrated':'select-mode-semantic';target.focus?.();target.click?.();}else if(!modeReadback&&modeTrigger&&!triggerExpanded&&!recentAction){action='open-mode-menu';toggleMenu(modeTrigger,true);}if(action){try{sessionStorage.setItem(modeKey,JSON.stringify({at:Date.now(),action,requested:requestedMode}));}catch(_){}modeReadback=false;}"
                + "const diagnostics={requested:requestedMode,currentMode,triggerCurrent,modeCandidates:rawModeControls.length,optionCandidates:visibleOptions.length,triggerFound:!!modeTrigger,triggerExpanded,targetFound,targetSelected,targetSource,selectedModes,recentAction,action,composer:!!composer,finalReadback:modeReadback};const modeDiag=()=>('requested='+requestedMode+';source='+targetSource+';current='+currentMode+';trigger='+(modeTrigger?1:0)+';targetFound='+(targetFound?1:0)+';targetSelected='+(targetSelected?1:0)+';attempt='+(action||'none')+';readback='+(modeReadback?1:0));"
