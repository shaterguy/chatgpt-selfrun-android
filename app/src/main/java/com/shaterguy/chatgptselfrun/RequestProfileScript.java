...
final class RequestProfileScript {
    static final String ENGINE_VERSION = "profile-registry-v1";
    ...
    static String documentStartScript() {
        return """
                (()=>{
                  ...
                  const TARGET_STORE='selfrun-drive:request-profile-target:v2';
                  ...
                  const targetValid=t=>{
                    ...
                    if(t.mode==='chat')return t.ready===false?(t.model===''&&t.reasoning===''):!!resolveProfile('chat','',t.reasoning)&&t.ready===true;
                    if(t.mode==='work'){
                  ...
                  const restoreTarget=()=>{try{const raw=localStorage.getItem(TARGET_STORE);if(!raw)return null;const t=JSON.parse(raw);if(!targetValid(t)){localStorage.removeItem(TARGET_STORE);return null;}return{mode:t.mode,model:t.model,reasoning:t.reasoning,runId:t.runId,ready:t.ready};}catch(_){return null;}};
                  ...
                  const begin=(mode,runId)=>{refreshRegistry();const m=norm(mode);if(m!=='chat'&&m!=='work')fail('unsupported_mode');state.target={mode:m,model:'',reasoning:'',runId:String(runId||'').slice(0,128),ready:false};persistTarget();state.last={ok:true,reason:'target_begun',mode:m};return true;};
                  const requireTarget=mode=>{const t=state.target;if(!t||t.mode!==mode)fail('target_mode_not_initialized');return t;};
                  const setChatReasoning=reasoning=>{refreshRegistry();const t=requireTarget('chat'),r=norm(reasoning);if(!resolveProfile('chat','',r))fail('unsupported_chat_reasoning');t.model='';t.reasoning=r;t.ready=true;persistTarget();state.last={ok:true,reason:'target_ready',mode:'chat',reasoning:r};return true;};
                  ...
                  const plan=()=>{refreshRegistry();const t=state.target;if(!t||!t.ready)fail('target_not_ready');const p=resolveProfile(t.mode,t.model,t.reasoning);if(!p)fail('profile_deleted_or_unsupported');return p.operations;};
                  ...
                  window.__selfRunRequestProfileEngine={version:__ENGINE_VERSION__,installRegistry,begin,setChatReasoning,setWorkModel,setWorkReasoning,armCapture,cancelCapture,consumeCapture,diagnostics:()=>({...state.last}),target:()=>state.target?{mode:state.target.mode,model:state.target.model,reasoning:state.target.reasoning,ready:state.target.ready}:null};
                })();
